...
procedure ExportarBinarioATexto(var archivoLogico:archivo; var archivoTexto: Text);
	var
		e: empleado;
	begin
		...
		while(not eof(archivoLogico)) do begin
			...
			writeln(archivoTexto, e.apellido, ' ', e.nombre, ' ', e.nroEmpleado, ' ', e.edad, ' ', e.dni, ' ');
		end;
		close(archivoLogico);
		close(archivoTexto);
...
procedure ExportarDNIFaltante(var archivoLogico:archivo; var archivoTextoDNI: Text);
	var
		e:empleado;
	begin

		reset(archivoLogico);
		assign(archivoTextoDNI, 'faltaDNIEmpleado.txt');
		rewrite(archivoTextoDNI);
		while (not eof(archivoLogico)) do begin
			read(archivoLogico,e);
			if (e.dni = 0) then begin {elegí al inicio del ejercicio usar INT para dni, pero debería ser un string}
				writeln(archivoTextoDNI,e.nombre, ' ', e.apellido, ' ', e.nroEmpleado, ' ', e.edad, ' ', e.dni);
			end;
		
		end;
		close(archivoTextoDNI);
		close(archivoLogico);
	end;
...
Var
	...
	archivoNombre1: cadena20;
	archivoLogico: archivo;
	archivoTexto, archivoTextoDNI: Text;
	opcion: string[1];
	
Begin
	write('Ingrese el nombre del archivo con el que desea trabajar: ');
	readln(archivoNombre1);
	assign(archivoLogico,archivoNombre1);
	...
	writeln('Ingrese "e" para exportar el contenido del archivo de empleados a un archivo de texto.');
	writeln('Ingrese "f" para exportar los empleados que carecen de DNI a un archivo de texto.');
	...
	while (opcion <> '0') do begin
		...
		case opcion of
			...
			'c': AgregarAlFinal(archivoLogico);
			'd': ActualizarMaestroDetalle(archivoLogico);
			'e': ExportarBinarioATexto(archivoLogico,archivoTexto);
			'f': ExportarDNIFaltante(archivoLogico,archivoTextoDNI);
			else
